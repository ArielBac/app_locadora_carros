      ...
      <tbody>
        <tr v-for="(obj, chave) in dadosFiltrados" :key="chave">
          <td v-for="(valor, chaveValor) in obj" :key="chaveValor">
            ...
            <span
              v-if="
                titulos[chaveValor].tipo == 'checkBoxDisponivel' && valor == '1'
              "
              >{{ "Disponível" }}</span
            >
            ...
            <span
              v-else-if="
                titulos[chaveValor].tipo == 'checkBoxDisponivel' && valor == '0'
              "
              >{{ "Indisponível" }}</span
            >

            <span
              v-if="titulos[chaveValor].tipo == 'marca' && valor !== null"
              >{{ valor.nome }}</span
            ><!-- A segunda condição do v-if verifica se existe uma marca associada a o determinado modelo-->
            <span
              v-if="titulos[chaveValor].tipo == 'modelo' && valor !== null"
              >{{ valor.nome }}</span
            ><!-- A segunda condição do v-if verifica se existe um modelo associada a o determinado modelo-->

            <span v-if="titulos[chaveValor].tipo == 'data'">{{
              valor | formataDataHoraGlobal
            }}</span>
            <!-- lógica de formatação feita na aula 385 -->
